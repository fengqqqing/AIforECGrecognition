...
本文档定义当前 ECG 上位机 demo 的运行契约：展示时跑什么、应该看到什么、怎样判断成功，以及失败时如何回退。它服务于“ECG 采集与初筛场景的 AI 辅助桌面工作台”实验演示，不把模型输出描述为最终临床诊断。

## 1. Demo 定位

当前 demo 目标是证明部署侧闭环可运行，并能支撑一条面向 ECG 采集与初筛人员的核心体验旅程：
...
演示重点是：用户先确认波形、导联和心率等基础状态，再查看 AI 辅助识别提示，最后通过运行指标、摘要和导出文件复盘本次运行。不要承诺医学准确率、自动临床诊断、临床处置优先级、患者管理、医院系统接入或真实临床可用性。
...
1. **启动固定场景**
   - 在仓库根目录执行 `.\run_demo.ps1`，或双击 `run_demo.bat`。
   - 主界面打开后，确认顶部模式徽标进入 Demo 启动或运行状态。

2. **确认信号输入**
   - 观察中央 ECG 波形区刷新，波形区是主视觉。
   - 讲述重点是：先确认当前有可观察的 ECG 信号，再谈 AI 辅助提示。

3. **观察基础状态**
   - 观察右侧 `心率` 卡更新为固定 demo 心率序列中的值。
   - 观察右侧 `导联状态` 卡保持三导联单通道连接语义。

4. **等待 AI 辅助识别**
   - 等待 2000 点固定窗口完成后，观察 `AI 辅助识别` 卡出现辅助识别结果。
   - 讲述时使用“辅助识别 / 辅助提示”，不要说成最终诊断。

5. **形成正式判读前的辅助提示**
   - 将 AI 结果解释为“是否需要优先关注或转交复核”的提示。
   - 不把它解释为临床处置、分诊或最终诊断依据。

6. **查看运行证据**
   - 观察底部运行指标区更新包计数、诊断次数、推理耗时、吞吐量和事件计数。
   - 等待回放完成后，顶部模式提示应显示已完成，点击 `最近摘要` 查看本次导出路径、指标、诊断记录和回放文件路径。

7. **导出与回放复盘**
   - 点击 `导出目录`，确认本次运行产物位于 `模型部署pyqt\ECGMonitor\runs\`。
   - 说明 `metrics.jsonl`、`diagnosis.csv`、`ecg_replay.csv` 分别用于指标、诊断事件和可回放 ECG 数据复盘。
...
1. 产品定位：这是面向 ECG 采集与初筛场景的 AI 辅助桌面工作台原型，不是临床独立诊断产品。
2. 用户旅程：固定 demo 模拟一次“信号输入 -> 状态确认 -> AI 辅助提示 -> 指标与导出复盘”的流程。
...
6. 可追溯导出：运行后导出 `metrics.jsonl`、`diagnosis.csv` 和 `ecg_replay.csv`，后续可继续回放。
7. 当前限制：真实模型 smoke 只证明部署链路可用；模型精度、临床数据、医生复核、权限审计、医院系统集成和 ECG 教学训练模式都不是 V1 已实现能力。
8. 后续方向：信号质量提示、结构化交接卡、复盘时间线和 ECG 回放训练模式可以作为产品路线讨论，但不要在当前 demo 中表述为已实现能力。
