...
1. 定位：这是 ECG 辅助识别实验系统，目标是展示部署闭环，不承诺临床独立诊断。
2. Demo：一键脚本启动固定样例回放，波形、心率、导联、辅助识别和指标同步更新。
3. 契约：部署侧读取模型契约，避免窗口长度、归一化参数和标签顺序漂移。
4. 管线：回放输入和串口输入最终进入同一条 ECG 处理管线，减少 GUI 与 CLI 行为分叉。
5. 追溯：运行后导出 metrics、diagnosis 和 replay 文件，方便复盘本次演示。
6. 边界：模型效果、真实临床数据、医生复核、权限审计和医院系统接入仍是后续方向。
